...
const submit = async () => {
  ...
  try {
    ...
    await stepUp.run(() => adminAPI.users.create({ ...form }))
...
watch(() => props.show, (v) => { if(v) Object.assign(form, { email: '', password: '', username: '', notes: '', role: 'user', concurrency: 1, rpm_limit: 0 }) })
