...
func TestContentModerationRuntimeSnapshotRefreshFailureKeepsStaleConfig(t *testing.T) {
	...
	// Expire the snapshot explicitly. A nanosecond TTL is not deterministic on
	// Windows because wall-clock resolution can make two adjacent time.Now
	// calls equal, preventing the stale-while-revalidate branch from running.
