...
func validateJSONSchemaValue(value any, schema map[string]any, path string) error {
	if alternatives, ok := schema["anyOf"].([]any); ok {
		if !matchesAnySchema(value, alternatives, path) {
	...
	if alternatives, ok := schema["oneOf"].([]any); ok {
		if !matchesAnySchema(value, alternatives, path) {
			return fmt.Errorf("value does not match oneOf at %s", path)
		}
		return nil
	}
...
func matchesAnySchema(value any, alternatives []any, path string) bool {
