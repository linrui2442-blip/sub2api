...
func TestSQLiteAuditRetentionKeepsSevenDayBoundaryAndUsage(t *testing.T) {
	drv, db, err := openPersonalSQLite("file:sqlite_audit_retention?mode=memory&cache=shared")
	require.NoError(t, err)
	client := ent.NewClient(ent.Driver(drv))
	t.Cleanup(func() { require.NoError(t, client.Close()) })
	require.NoError(t, client.Schema.Create(context.Background()))
	...
	auditRepo := NewAuditLogRepository(db)
	inserted, err := auditRepo.BatchInsert(context.Background(), []*service.AuditLog{
		{CreatedAt: now.Add(-8 * 24 * time.Hour), Action: "old"},
		{CreatedAt: now.Add(-6 * 24 * time.Hour), Action: "recent"},
		{CreatedAt: cutoff, Action: "boundary"},
	})
	require.NoError(t, err)
	require.Equal(t, int64(3), inserted)

	usageRepo := newUsageLogRepositoryWithSQL(client, db)
	usage := personalUsageLog("audit-retention-usage", false)
	created, err := usageRepo.createSingle(context.Background(), db, usage)
	require.NoError(t, err)
	require.True(t, created)

	deleted, err := auditRepo.DeleteBefore(context.Background(), cutoff, 100)
	require.NoError(t, err)
	require.Equal(t, int64(1), deleted)

	var auditCount, usageCount int
	require.NoError(t, db.QueryRow("SELECT COUNT(*) FROM audit_logs").Scan(&auditCount))
	require.NoError(t, db.QueryRow("SELECT COUNT(*) FROM usage_logs").Scan(&usageCount))
	require.Equal(t, 2, auditCount)
	require.Equal(t, 1, usageCount, "audit retention must not modify usage data")
}
