	...
	for _, tt := range []struct {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			router.POST("/v1/messages", func(c *gin.Context) {
				...
				c.Next()
			}, personalUnifiedRouterMiddleware("", servermiddleware.AnthropicErrorWriter), func(c *gin.Context) {
...
func TestPersonalUnifiedGeminiRouteUsesEndpointHintAndStripsExplicitNamespace(t *testing.T) {
	...
	for _, tt := range []struct {
		...
		router.POST("/v1beta/models/*modelAction", func(c *gin.Context) {
			...
			c.Next()
		}, personalUnifiedRouterMiddleware(service.PlatformGemini, servermiddleware.GoogleErrorWriter), func(c *gin.Context) {
