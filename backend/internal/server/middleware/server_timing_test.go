...
func TestServerTimingScopesAndRoleGate(t *testing.T) {
	tests := []struct {
		...
		{name: "removed payment path", enabled: true, path: "/api/v1/payment/plans", role: "user"},
		{name: "removed operations path", enabled: true, path: "/api/v1/channel-monitors/1/status", role: "user"},
	}
